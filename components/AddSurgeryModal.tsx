...
interface AddSurgeryModalProps {
  visible: boolean;
  onClose: () => void;
  onSubmit: (surgery: SurgeryData) => void;
  petId: string;
  userId: string;
...
const STATUS_OPTIONS: SurgeryStatus[] = ['recovered', 'recovering', 'complication'];
...
const AddSurgeryModal: React.FC<AddSurgeryModalProps> = ({
  ...
  const [surgeryStatus, setSurgeryStatus] = useState<SurgeryStatus>('recovered');
  ...
  const applyOcrResult = useCallback((result: OcrSurgeryResult) => {
    ...
    if (result.anesthesia) setAnesthesia(result.anesthesia);
    if (result.status) setSurgeryStatus(result.status);
  ...
  const validate = useCallback((): boolean => {
    ...
    if (!name.trim()) {
      newErrors.name = t('health.surgeryNameRequired');
    }
    if (!date.trim()) {
  ...
  const handleSubmit = useCallback(() => {
    ...
    onSubmit(surgery);
  }, [validate, name, date, veterinarian, clinic, anesthesia, surgeryStatus, notes, petId, userId, onSubmit]);

  if (!visible) return null;

  const statusLabelKey: Record<SurgeryStatus, string> = {
    recovered: 'health.recovered',
    recovering: 'health.surgeryRecovering',
    complication: 'health.surgeryComplication',
  };
    ...
    <View style={styles.stepContainer}>
      <View style={styles.headerRow}>
        <View style={styles.titleRow}>
          <Scissors size={rs(22)} color={colors.accent} strokeWidth={1.8} />
          <Text style={styles.title}>{t('health.addSurgery')}</Text>
        </View>
      ...
      <Text style={styles.subtitle}>{t('health.surgeryMethodQuestion')}</Text>
      ...
      <TouchableOpacity style={styles.methodCard} onPress={handleTakePhoto} activeOpacity={0.7}>
        ...
        <View style={styles.methodTextWrap}>
          <Text style={styles.methodTitle}>{t('health.photoSurgery')}</Text>
          <Text style={styles.methodDesc}>{t('health.photoSurgeryDesc')}</Text>
        </View>
      ...
      <TouchableOpacity style={styles.methodCard} onPress={handlePickFromGallery} activeOpacity={0.7}>
        ...
        <View style={styles.methodTextWrap}>
          <Text style={styles.methodTitle}>{t('health.gallerySurgery')}</Text>
          <Text style={styles.methodDesc}>{t('health.gallerySurgeryDesc')}</Text>
        </View>
        <ArrowRight size={rs(18)} color={colors.accent} strokeWidth={1.8} />
      </TouchableOpacity>
      ...
      <TouchableOpacity style={styles.methodCard} onPress={handleManualEntry} activeOpacity={0.7}>
        ...
        <View style={styles.methodTextWrap}>
          <Text style={styles.methodTitle}>{t('health.manualSurgeryEntry')}</Text>
          <Text style={styles.methodDesc}>{t('health.manualSurgeryEntryDesc')}</Text>
        </View>
        <ArrowRight size={rs(18)} color={colors.accent} strokeWidth={1.8} />
      </TouchableOpacity>
    </View>
  ...
  const renderStep1 = () => (
    <View style={styles.stepContainer}>
      <View style={styles.headerRow}>
        ...
        <Text style={styles.title}>{t('health.surgeryDetails')}</Text>
        <TouchableOpacity onPress={handleClose} hitSlop={{ top: 12, bottom: 12, left: 12, right: 12 }}>
          <X size={rs(22)} color={colors.accent} strokeWidth={1.8} />
        </TouchableOpacity>
      </View>
      ...
      {analyzing && (
        <Animated.View style={[styles.analyzingBanner, { transform: [{ scale: pulseAnim }] }]}>
          ...
          <Text style={styles.analyzingText}>{t('health.analyzingSurgery')}</Text>
      ...
      >
        <Input
          label={t('health.surgeryName')}
          placeholder={t('health.surgeryNamePlaceholder')}
          icon={<Scissors size={rs(18)} color={colors.petrol} strokeWidth={1.8} />}
          value={name}
        ...
        <Input
          label={t('health.anesthesia')}
          placeholder={t('health.anesthesiaPlaceholder')}
          icon={<HeartPulse size={rs(18)} color={colors.petrol} strokeWidth={1.8} />}
          value={anesthesia}
          onChangeText={setAnesthesia}
        />
        ...
        <View style={styles.chipSection}>
          ...
          <View style={styles.chipRow}>
            {STATUS_OPTIONS.map((s) => (
              <TouchableOpacity
                key={s}
                style={[styles.chip, surgeryStatus === s && styles.chipActive]}
                onPress={() => setSurgeryStatus(s)}
                activeOpacity={0.7}
              >
                <Text style={[styles.chipText, surgeryStatus === s && styles.chipTextActive]}>
                  {t(statusLabelKey[s])}
                </Text>
              </TouchableOpacity>
            ))}
        ...
        <Input
          ...
          placeholder={t('health.surgeryNotesPlaceholder')}
        ...
        >
          {isSubmitting ? (
            ...
            <>
              <Text style={styles.submitBtnText}>{t('health.saveSurgery')}</Text>
