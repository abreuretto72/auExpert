    ...
    <div style={{
      ...
      <div ref={containerRef} style={{
        ...
        <div style={{ padding: "26px 22px 6px" }}>
          <p style={{ color: C.textDim, fontSize: 14, margin: "0 0 4px", fontFamily: font, fontWeight: 500 }}>Bem-vinda de volta,</p>
          <h2 style={{ color: C.text, fontSize: 28, fontWeight: 700, margin: 0, fontFamily: font }}>Ana!</h2>
        </div>

        {/* ===== PET COUNT BANNER ===== */}
        <div style={{
          margin: "20px 20px 0", padding: "18px 22px",
          background: `linear-gradient(135deg, ${C.accent}10, ${C.accent}04)`,
          borderRadius: 18, border: `1px solid ${C.accent}18`,
          display: "flex", alignItems: "center", justifyContent: "space-between",
        }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            {I.heart(20, C.accent)}
            <span style={{ color: C.text, fontSize: 15, fontWeight: 700, fontFamily: font }}>{petsData.length} pets cadastrados</span>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            {petsData.map(p => (
              <div key={p.id} style={{ width: 32, height: 32, borderRadius: 10, background: p.color + "15", display: "flex", alignItems: "center", justifyContent: "center" }}>
                {p.species === "dog" ? I.dog(18, p.color) : I.cat(18, p.color)}
              </div>
            ))}
          </div>
        </div>
